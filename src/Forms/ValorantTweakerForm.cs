...
internal sealed class ValorantTweakerForm : Form
{
    ...
    private void WireEvents()
    {
        ...
        dashboardTabButton.Click += async (_, _) => await ShowDashboardPageAsync();
    ...
    private async System.Threading.Tasks.Task ShowDashboardPageAsync()
    {
        if (string.Equals(_activePageKey, DashboardPageKey, StringComparison.Ordinal) &&
            contentHost.Controls.Count > 0 &&
            contentHost.Controls[0] is DashboardPage visibleDashboard &&
            !visibleDashboard.IsDisposed)
        {
            SetActiveTab(dashboardTabButton);
            RestorePageVisualState(DashboardPageKey, visibleDashboard);
            return;
        }
        ...
        else if (string.Equals(_activePageKey, TelemetryPageKey, StringComparison.Ordinal))
        {
            AttachPageStatic(dashboard, dashboardTabButton, DashboardPageKey);
            return;
        }
        else
        {
            try
            {
                await UiAnimator.AnimatePageTransitionAsync(contentHost, dashboard, _ctsTransition.Token);
            }
            catch (OperationCanceledException)
            {
                return;
        ...
        if (previousDashboard is not null &&
            ...
            previousDashboard.Parent is null)
        {
    ...
    private void RestorePageVisualState(string pageKey, Control page)
    {
        ...
        if (string.Equals(pageKey, TelemetryPageKey, StringComparison.Ordinal) &&
            page is TelemetryPage telemetryPage)
        {
            telemetryPage.RestoreVisualState();
            FlushPendingTerminalLines();
    ...
    private void SetActiveTab(Button selectedButton)
    {
        ...
        foreach (var button in sidebarButtons)
        {
            ...
            {
                navButton.InactiveTextColor = inactiveTextColor;
                navButton.HoverTextColor = Color.White;
                navButton.SelectedTextColor = Color.White;
                navButton.InactiveIconColor = inactiveTextColor;
                navButton.HoverIconColor = Color.White;
                navButton.IsSelected = ReferenceEquals(button, selectedButton);
            }
            else
            {
                button.BackColor = ReferenceEquals(button, selectedButton)
                    ? Color.FromArgb(55, 55, 57)
                    : Color.Transparent;
                button.ForeColor = ReferenceEquals(button, selectedButton)
                    ? Color.White
                    : inactiveTextColor;
                button.Invalidate();
            }
    ...
    private Control CreateModulesPage()
    {
        ...
        page.Controls.Add(CreateCard("M?dulos do Sistema", CreateGroupedModulesPanel()), 0, 0);
        return page;
    }
    ...
    {
        return new TelemetryPage(btnABTest, performanceChart, CreateLogFrame())
        {
            Dock = DockStyle.Fill,
            Margin = new Padding(0)
        };
    ...
    private static TableLayoutPanel CreatePageGrid(int rows)
    {
        ...
        {
            page.RowStyles.Add(new RowStyle(SizeType.Percent, 100F / Math.Max(1, rows)));
        }
        return page;
    }
